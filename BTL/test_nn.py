...
from model_nn import L_model_forward
import queue
import threading
import time
...
MODEL_RUN = 'run_20260113_153527'  # Thay đổi theo run mới nhất của bạn
CHECKPOINT_FILE = 'final_model.npz'  # Sử dụng final model
...
def predict_command(audio, parameters, label_mapping):
    ...
    # Reshape để phù hợp với input của model
    # Model expects: (input_size, 1) where input_size = 32*32 = 1024
    # Resize spectrogram về 32x32 (giống với training)
    import cv2
    spectrogram_resized = cv2.resize(spectrogram, (32, 32), interpolation=cv2.INTER_AREA)
    
    # Flatten và reshape
    X = spectrogram_resized.flatten().reshape(-1, 1)
    ...
    # Lấy prediction
    pred_idx = np.argmax(probs, axis=0)[0]
...
def continuous_voice_command_recognition():
    ...
    # Load model
    run_dir = os.path.join(CHECKPOINT_DIR, MODEL_RUN)
...
def single_test():
    ...
    """
    print("=" * 70)
    print("🎯 VOICE COMMAND RECOGNITION - SINGLE TEST")
    print("=" * 70)
